...
# Авторизация в ВК
vk_session = VkApi(token="") #токен не стали скидывать специально
vk = vk_session.get_api()

# Получение списка друзей пользователя
friends_list = vk.friends.get(user_id=165171730)["items"]
for friend_id in friends_list:
  if friend_id in my_group: # оставляем только нашу группу
    final_friends_list.append(friend_id)
final_friends_list.append(165171730) # добавляем пользователя, чьих друзей отбирали выше
