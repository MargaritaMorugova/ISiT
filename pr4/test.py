...
def test_model(model, initial_state):
    ...
    while not done:
        ...
        # Выводим информацию о действии с измененным порядком индексов
        if info['transfer_amount'] < 0:
            print(f"Шаг {steps + 1}: Перелито {-1 * info['transfer_amount']} из стакана {info['to_idx'] + 1} в стакан {info['from_idx'] + 1}")
        else:
            print(f"Шаг {steps + 1}: Перелито {info['transfer_amount']} из стакана {info['from_idx'] + 1} в стакан {info['to_idx'] + 1}")
